...
Apply functions to detect upper airways artifacts on the measurements of child ID45264.
...
# Starting datasets
```{r}
TC_ID45264 <- data.frame()
RV_ID45264 <- data.frame()
count <- 0
```
...
```{r}
source_rmd(here("Functions_ArtifactDetection.Rmd"))
```
...
```{r}
top <- read_excel(here("input/ID45264_Vol,Pcyl,Flow.xlsx"), 
...
bottom <- read_excel(here("input/ID45264_Vol,Pcyl,Flow.xlsx"), 
...
```{r}
top <- readxl::read_excel(here("input/ID45264_Timecourse Data.xlsx"),  
...
dt <- readxl::read_excel(here("input/ID45264_Timecourse Data.xlsx"),  
...
TC_ID45264 <- TC_ID45264 %>% bind_rows(TC)
RV_ID45264 <- RV_ID45264 %>% bind_rows(RV)
```
...
```{r}
TC_ID45264<-T.AllSteps(TC_ID45264)
```
...
```{r}
name <- paste0(TC_ID45264$patient[1], "_", as.Date(TC_ID45264$timestamp[1]), "_TC_withQC.rds")
write_rds(TC_ID45264, here("1_Artifact-Detection/output", name))

name <- paste0(RV_ID45264$patient[1], "_", as.Date(RV_ID45264$timestamp[1]), "_RV_withQC.rds")
write_rds(RV_ID45264, here("1_Artifact-Detection/output", name))
```
...
```{r}
TC_ID45264 %>% count(patient_test, MeasureWithinTest, KeepMeasure.SD, KeepMeasure.SDExt, KeepMeasure.TSDExt)
```
