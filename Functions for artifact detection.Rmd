...
M.AllPlots <- function(M.VolFlow, M.Impedance){
 ...
 name <- paste0(M.Impedance$hhid, "_",  
                M.Impedance$patient_test, "_", M.Impedance$MeasureWithinTest, "_",
                as.Date(M.Impedance$timestamp), "_", M.Impedance$Visit, "M.png")
 
 
 PM <- P4 + P3+ P5 +P2+P1 + plot_layout(nrow = 5, byrow = FALSE) + 
   plot_annotation(title=name, subtitle=M.Impedance$Timestamp)
 
 ggsave(here("output_visualizations", name), PM, height = 10, width = 8)
...
T.OutZ7 <- function(Ms.Impedance){
  ...
  BreathsToDrop <- Ms.Impedance %>% 
    merge(Limits, by=c("hhid", "patient_test"), all = T) %>% 
    mutate(BreathsToDrop = case_when(
      Z7>T.OutlierLimitZ7.H ~ 1,
      Z7<T.OutlierLimitZ7.L ~ 1,
      TRUE ~ 0)) %>% 
    filter(BreathsToDrop==1) %>% 
    mutate(ID = paste0(hhid,"-", patient_test, "-", MeasureWithinTest, "-", Breath)) %>% 
...
validMeasures<-Ms.Impedance %>% 
  ...
  distinct(hhid, patient_test, MeasureWithinTest, BreathAfterTrim) %>% 
  count(hhid, patient_test, MeasureWithinTest) %>% 
  rename("ValidBreaths"=n) %>% 
  filter(ValidBreaths>=4) %>% 
  mutate(ID = paste0(hhid,"-", patient_test, "-", MeasureWithinTest)) %>% 
  select(ID) %>% t() %>% as.character()
